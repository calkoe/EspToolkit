...
void spi_master_init(SSD1306_t * dev, int16_t GPIO_MOSI, int16_t GPIO_SCLK, int16_t GPIO_CS, int16_t GPIO_DC, int16_t GPIO_RESET)
{
	...
	gpio_pad_select_gpio( (gpio_num_t)GPIO_DC );
	gpio_set_direction( (gpio_num_t)GPIO_DC, (gpio_mode_t)GPIO_MODE_OUTPUT );
	gpio_set_level( (gpio_num_t)GPIO_DC, 0 );

	if ( GPIO_RESET >= 0 ) {
		gpio_pad_select_gpio( (gpio_num_t)GPIO_RESET );
		gpio_set_direction( (gpio_num_t)GPIO_RESET, (gpio_mode_t)GPIO_MODE_OUTPUT );
	...
	spi_bus_config.sclk_io_num = GPIO_SCLK;
	spi_bus_config.mosi_io_num = GPIO_MOSI;
